...
- ✅ **PC-106** `[P2]` — Clean up repository root.
  *Rationale:* `bench align.chain`, `road 01 align.chain`, `NWP-*.chain`, `models - Copy.xlsx`, `workflow-template (5).json`, and the loose `.12dattmf` files look like test fixtures living at the repo root. Move them to `samples/` or `.gitignore` them. `fastapi-venv/` and `venv/` under `backend/` should never be committed. **(Finished 2026-06-03):** the `.chain`/xlsx fixtures had been gitignored, but a tracked `Node-based Workflow UI Design/` prototype bundle (its own package.json + UI components), the two `*.12dattmf` files, and a scratch `Feature.md` had been missed — now untracked (kept on disk, added to `.gitignore`).
...
- **PC-506** `[P2]` `[Size: M]` `[Mode: regular]` — Broaden the Playwright E2E coverage.
  *Rationale:* PC-505 covers the golden path, but most in-canvas features shipped "tsc + unit verified, not browser-clicked" (see nearly every EPIC-09/10 rationale, and the PC-705 light-mode screenshots). Add focused specs on the **existing** hermetic harness (port 3100, in-memory backend, tour suppression, `e2e/fixtures/e2e-models.xlsx`) for the highest-traffic interactions that unit tests can't reach: the per-source ▶ run + 🧪 test-run (PC-1003/1004), the Excel column picker modal (PC-704), and the Runs-page download + expiry (PC-906). Each converts a documented "not browser-tested" gap into real regression coverage. Add them to the existing `e2e` CI job. Keep specs independent so one flaky interaction doesn't cascade.
...
- ✅ **PC-604** `[P2]` — Consolidate `start.sh` and `start.bat`, or delete both.
  *Rationale:* Both scripts have been removed. The README and CLAUDE.md document the commands directly. **(Finished 2026-06-03):** despite this note the two `backend/start.{sh,bat}` were still tracked and on disk — now `git rm`'d.

- ✅ **PC-605** — Finish the README cleanup PC-602 started.
  *Rationale:* PC-602 left residual staleness: the Quick Start used hardcoded absolute `G:\…` drive paths (now relative `cd backend` / `cd frontend`), and the Project Structure + an "Extended Format (Legacy System)" Excel section still referenced the legacy batch path and files (`pychain_service.py`, `lib/api.ts`) that were deleted with PC-101. Removed/updated those so the README matches the shipped single-workflow system, and pointed the Excel-format note at the PC-704 column picker.
...
## EPIC-11 — Interop & Authoring Confidence

The build → run → download loop is solid. These two raise confidence in what you author and let you bring existing work in. Both treat the command generators in `backend/commands/` as the canonical spec — PC-1101 inverts them, PC-1102 reasons across them.

- **PC-1101** `[P1]` `[Size: L]` `[Mode: superpowers]` — Import an existing `.chain` file into the graph.
  *Rationale:* Users already have libraries of hand-authored `.chain` files (the repo root alone shipped several). Today the only direction is graph → `.chain`; there is no way to bring an existing chain back in to edit it — the single biggest "do I have to rebuild everything by hand?" adoption barrier. Add an importer that parses a `.chain` and reconstructs a node graph: recognise each 12d command, map it back to its node type + parameters (the inverse of the `commands/<category>/*.py` generators), drop a `foreachModel` + `chainFileOutput` scaffold, auto-layout via PC-902 (dagre), and wire flow edges in document order. **Scope v1 honestly:** handle the node types PyChain already generates; emit a clear "N commands not recognised → imported as sticky-note placeholders so nothing is silently dropped" report; round-trip is best-effort, **not** guaranteed lossless. `superpowers` because parsing the intentionally-malformed single-line 12d XML and mapping it back without corrupting the graph is the risky seam — worth a brainstorm + TDD-first with golden fixtures (round-trip a generated chain back to a graph and compare).

- **PC-1102** `[P2]` `[Size: M]` `[Mode: feature-dev]` — Pre-run semantic / relational graph linter.
  *Rationale:* PC-703 validates each node's own fields and PC-911 makes those errors actionable, but both are **per-node**. The gap is **cross-node** consistency the compiler can catch *before* 12d ever runs: an `addModelToView` / `removeModelFromView` that names a view no `createView` produces; an `import` with no resolvable file path; a `chainFileOutput` whose `projectFolder` variable is never set by any `setVariable`; a subgraph orphaned from the `foreachModel` → `chainFileOutput` path. Extend `validateWorkflow` (the pre-run gate, not the per-node `validateNode`) with these relational checks, reusing PC-911's `ActionableError` shape (name the offending node, suggest the fix, `focusNodeId` to scroll to it). Frontend-only, compile-time — the backend stays untouched. Builds directly on PC-703 + PC-911.
...
25. ✅ **PC-605** — Finish the README cleanup (residual legacy refs + drive paths).

### Phase 7 — Interop & authoring confidence (the suggested next non-gated work)
26. **PC-1101** — Import an existing `.chain` into the graph. The adoption unlock — bring existing chains in to edit. Brainstorm first; round-trip golden fixtures.
27. **PC-1102** — Pre-run semantic linter. Cheap follow-on; catches cross-node mistakes (view-not-created, missing import path, undefined `projectFolder`) before 12d does.
28. **PC-506** — Broaden the Playwright E2E. Lock in the in-canvas features (per-source run, column picker, Runs page) that are currently unit-only.

### Phase 8 — Production hardening (only if going multi-user)
29. **PC-802** — File size and rate limits.
30. **PC-801** — Supabase Auth + RLS. The big one — do not start until you've decided to make this a team tool.
